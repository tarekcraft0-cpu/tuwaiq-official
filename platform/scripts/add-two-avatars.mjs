...
const shots = [
  {
    username: "0ZZ0",
    file:
      "c__Users_Alsafy_AppData_Roaming_Cursor_User_workspaceStorage_empty-window_images_WhatsApp_Image_2026-08-06_at_1.30.34_AM-e1a54b26-dee2-4003-a4da-ea32ec46eca3.png",
  },
  {
    username: "Alphavill",
    file:
      "c__Users_Alsafy_AppData_Roaming_Cursor_User_workspaceStorage_empty-window_images_WhatsApp_Image_2026-08-06_at_1.31.21_AM-a6e02683-e87f-4fdd-8bfd-26ad739f88ca.png",
  },
];
...
async function findCircle(imgPath) {
  ...
  for (let r = 55; r <= 95; r += 2) {
    for (let cy = Math.round(h * 0.18); cy <= Math.round(h * 0.42); cy += 3) {
...
async function saveAvatar(imgPath, username, best) {
...
for (const shot of shots) {
  const full = path.join(ASSETS, shot.file);
  const best = await findCircle(full);
  console.log(shot.username, best);
  const avatar = await saveAvatar(full, shot.username, best);
  map[shot.username.toLowerCase()] = {
    username: shot.username,
    avatar,
  };
  if (!usersTs.includes(`"${shot.username}"`)) {
